          ...
          <CommandGroup heading={`${isSearchMode ? 'Search results' : 'Popular stocks'} (${displayStocks?.length || 0})`}>
            {displayStocks?.map((stock) => (
              <CommandItem
                ...
                onSelect={() => {
                  handleSelectStock();
                  console.log(`Navigating to /stocks/${stock.symbol}`);
                }}
                className="search-item"
              >
                <Link
                  href={`/stocks/${stock.symbol}`}
                  className="search-item-link flex items-center w-full"
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-500">{stock.exchange} | {stock.type}</div>
                  </div>
                </Link>
              </CommandItem>
            ))}
